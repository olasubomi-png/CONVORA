...
export function FacebookSettings({
  ...
  function create() {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) {
        setError(data.error?.message ?? "Failed to connect Facebook");
        return;
      }
    ...
    <div className="space-y-8">
      <div className="border border-[#e4e4e2] bg-white p-5 text-sm">
        ...
        <p className="mt-2 text-xs text-[#5c5c5c]">
          Subscribe the Page to messages in Meta Developer Console. Secrets are
          encrypted at rest and never returned after save.
        </p>
      </div>

      <div className="border border-[#e4e4e2] bg-white p-5">
        <h2 className="text-sm font-medium">Connected Pages</h2>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {installations.map((i) => (
              ...
              >
                <span>
                  {i.displayName}
                  {typeof i.publicConfig.pageId === "string"
                    ? ` · Page ${i.publicConfig.pageId}`
                    : ""}
                </span>
      ...
      <div className="border border-[#e4e4e2] bg-white p-5 space-y-3">
        <h2 className="text-sm font-medium">Connect Facebook Page</h2>
        {error ? (
